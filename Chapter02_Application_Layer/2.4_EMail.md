...
* **动作**：
  ...
  * **发送方**：不在ASCII字符 -> base64编码 -> 在ASCII字符的字符（一个映射关系）
  * **接收方**：编码后的字符 -> base64编码 -> 解释出原不在ASCII字符 -> MIME解释出来
...
| **IMAP** | Internet邮件访问协议(Internet Mail Access Protocol) | RFC 1730 | 更多特性(远程目录维护，远程搬运报文到不同邮箱)，更复杂，在服务器上处理存储的报文 |
...
* **POP3特点**(本地管理文件夹)：
  * 先前的例子使用**下载并删除**模式。
    * 如果改变客户机，Bob不能阅读邮件，因为文件删除了
  * 可以切换到**下载并保留**：不同客户机上为报文的拷贝
  * POP3在会话中是无状态的，因为不支持远程目录维护
* **IMAP特点**(远程管理文件夹):
  * IMAP服务器将每个报文与一个文件夹联系起来
    * 允许用户用目录来组织报文
    * 允许用户读取报文组件
  * IMAP在会话过程中**保留**用户状态，**维护**远程目录：
    * 目录名、报文ID与目录名之间映射
