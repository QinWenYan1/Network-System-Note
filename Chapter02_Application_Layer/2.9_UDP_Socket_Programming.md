...
|------------------------|--------|
| create socket, port=x, for incoming request: `serverSocket = socket(PF_INET, SOCK_DGRAM, 0)` | Create clientSocket: `clientSocket = socket(PF_INET, SOCK_DGRAM, 0)` |
| `bind(serverSocket, &sad, ...)` | |
| read request from serverSocket: `n=recvfrom(serverSocket, ..., &cad, ...)` | send datagram request using clientSocket: `n=sendto(clientSocket, ..., &sad, ...)` |
| write reply to serverSocket specifying client host address, port number: `n=sendto(serverSocket, ..., &cad, ...)` | read reply from clientSocket: `n=recvfrom(clientSocket, ..., &sad, ...)` |
| | close clientSocket |
