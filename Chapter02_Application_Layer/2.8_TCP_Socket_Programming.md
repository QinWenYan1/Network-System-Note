...
|------|----------------------|--------|
| 1.创建 | create socket, port=x, for incoming request: `welcomeSocket = Socket(...)` | - |
| 2.绑定 | `bind(welcomeSocket, &sad, ...)` | - |
| 3.监听 | `listen(welcomeSocket, ...)` | - |
| 4.等待 | wait for incoming connection request | create socket, connect to hostid, port=x: `ClientSocket = socket(PF_INET,...)` |
| 5.连接 | `connectionSocket = accept(welcomeSocket, &cad, ...)` | `connect(clientSocket, &sad ...)` |
| 6.请求 | read request from `connectionSocket` | send request using `clientSocket` |
| 7.响应 | write reply to `connectionSocket` | read reply from `clientSocket` |
| 8.关闭 | close `connectionSocket` | close `clientSocket` |
