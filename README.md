# 📡 Network System Note 

> 计算机网络秋招面试复习笔记 

---

<details open>
<summary><h2>📋 关于本仓库</h2></summary>

### 🎯 目的

系统学习计算机网络核心知识，整理面试高频考点，配合实验验证理解，为秋招技术面试做好充分准备。

### 📝 内容

- **理论笔记**：基于中科大视频课程和《自顶向下》教材的知识点整理
- **实验报告**：Kurose官网Wireshark Labs的实验记录和分析
- **面试八股**：各章节面试高频问题及答案
- **原始资料**：课程PPT及标注版本

### 👥 面向人群

- 准备秋招/春招的后端开发求职者
- 需要系统复习计算机网络的学生
- 希望通过实验加深理解的学习者

</details>

---

<details open>
<summary><h2>🧭 章节导航</h2></summary>

| 章节 | 标题 | Lab |
|------|------|-----|
| [Chapter01](./Chapter01_Computer_Networks_overview/README.md) | 计算机网络概述 | - |
| [Chapter02](./Chapter02_Application_Layer/README.md) | 应用层 | Lab2-HTTP |
| [Chapter03](./Chapter03-Transport-Layer/) | 传输层 | Lab3-TCP |
| [Chapter04](./Chapter04-Network-Layer-Data-Plane/) | 网络层：数据平面 | - |
| [Chapter05](./Chapter05-Network-Layer-Control-Plane/) | 网络层：控制平面 | - |
| [Chapter06](./Chapter06-Link-Layer-and-LANs/) | 链路层和局域网 | - |
| [Chapter07](./Chapter07-Wireless-and-Mobile-Networks/) | 无线网络和移动网络 | - |
| [Chapter08](./Chapter08-Security-in-Computer-Networks/) | 网络安全 | - |
</details>

---

<details>
<summary><h2>📁 仓库结构</h2></summary>

```
Network-Interview-Notes/
│
├── 📁 Chapter01-Computer-Networks-and-Internet/
│   ├── 📄 README.md
│   ├── 📄 1.1-What-is-Internet.md
│   ├── 📄 1.2-Network-Edge.md
│   └── 📁 Lab/
│
├── 📁 Chapter02-Application-Layer/
│   ├── 📄 README.md
│   ├── 📄 2.1-Principles-of-Network-Apps.md
│   ├── 📄 2.2-The-Web-and-HTTP.md
│   ├── 📄 2.5-DNS.md
│   └── 📁 Lab/
│       └── 📄 Lab2-HTTP-Wireshark.md
│
├── 📁 Chapter03-Transport-Layer/
│   ├── 📄 README.md
│   ├── 📄 3.5-Connection-Oriented-Transport-TCP.md
│   └── 📁 Lab/
│       └── 📄 Lab3-TCP-Wireshark.md
│
├── 📁 Chapter04-Network-Layer-Data-Plane/
│   ├── 📄 README.md
│   └── 📁 Lab/
│
├── 📁 Chapter05-Network-Layer-Control-Plane/
│   ├── 📄 README.md
│   └── 📁 Lab/
│
├── 📁 Chapter06-Link-Layer-and-LANs/
│   ├── 📄 README.md
│   └── 📁 Lab/
│
├── 📁 Chapter07-Wireless-and-Mobile-Networks/
│   ├── 📄 README.md
│   └── 📁 Lab/
│
├── 📁 Chapter08-Security-in-Computer-Networks/
│   ├── 📄 README.md
│   └── 📁 Lab/
│
├── 📁 99-Interview-Eight-Part/
│   └── 📄 README.md
│
└── 📁 99-PPT-Materials/
    ├── 📁 Original/
    └── 📁 Annotated/
```
</details>

---

<details>
<summary><h2>📚 资源链接</h2></summary>

| 资源 | 链接 |
|------|------|
| **中科大视频课程** | https://www.bilibili.com/video/BV1JV411t7ow |
| **配套教材** | 《计算机网络：自顶向下方法》第7版 |
| **Kurose官网** | https://gaia.cs.umass.edu/kurose_ross/index.php |
| **Wireshark Labs** | https://gaia.cs.umass.edu/kurose_ross/wireshark.php |
</details>

---

