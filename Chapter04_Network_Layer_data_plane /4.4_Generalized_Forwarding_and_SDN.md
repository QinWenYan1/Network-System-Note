...
网络层(Network Layer)功能分为两大核心组件：

- **转发(Forwarding)**：对于从某个端口到来的分组，转发到合适的输出端口。这是**本地的、每个路由器**的功能，决定某个从某个端口进入的分组从哪个端口输出。类比于旅行中**一个多岔路口的进入和转出过程**。
- **路由(Routing)**：决定分组从源端到目标端的路径。这是**网络范围的逻辑**，决定分组端到端穿行于各个路由器的路径。类比于**规划从源到目标的旅行路径**，涉及**路由算法(Routing Algorithm)**。
...
**注意点**
- ⚠️ **核心区别**：转发是本地决策（每个路由器独立），路由是全局决策（端到端路径规划）。
- 💡 **旅行类比**：转发 = 在多岔路口选择出口；路由 = 出发前规划完整路线。
- 📋 **术语提醒**：转发(Forwarding) vs 路由(Routing)；数据平面(Data Plane) vs 控制平面(Control Plane)。
...
- **控制平面和数据平面分离(Control Plane and Data Plane Separation)** 是SDN的核心起点。
...
- 📋 **术语提醒**：流表(Flow Table)、PDU(Protocol Data Unit)、SDN交换机(SDN Switch)。
...
2. **集中式实现控制逻辑，网络管理容易**：
   ...
   - **避免路由器的误配置**。
3. **基于流表的匹配+行动(Match+Action)的工作方式**允许**"可编程的"分组交换机**：
   - 实现**流量工程(Traffic Engineering)**等高级特性。
   - 在此框架下实现各种新型（未来）的网络设备。

**注意点**
- ⚠️ **开放生态的关键**：控制平面开放实现意味着不再被单一厂商绑定，控制器、交换机、应用可由不同厂商提供。
...
**注意点**
- ⚠️ **核心矛盾**：传统IP路由是**面向目标(Destination-Based)**的，只根据目的IP做转发决策，无法根据源地址、应用类型、负载状况等做灵活调度。
- 🔄 **知识关联**：这三个"无法"正是SDN要解决的问题——通过流表的细粒度匹配+行动实现灵活控制。
...
**SDN特点(SDN Characteristics)**：

1. **通用"flow-based"基于流的匹配+行动**（e.g., OpenFlow）。
2. **控制平面和数据平面的分离(Separation of Control and Data Plane)**。
3. **控制平面功能在数据交换设备之外实现**（控制逻辑脱离硬件）。
4. **可编程控制(Programmable Control)**：
   - 在控制器之上以**网络应用(Network Applications)**形式实现各种网络功能：
     - 路由(Routing)
     - 接入控制(Access Control)
     - 负载均衡(Load Balancing)
     - 其他功能
...
- 📋 **术语提醒**：基于流的(Flow-Based)、网络应用(Network Applications)、可编程控制(Programmable Control)。
...
**SDN架构：数据平面交换机(SDN Architecture: Data Plane Switches)**：

- **数据平面交换机(Data Plane Switches)**：
  - 快速，简单，商业化交换设备。
  - 采用**硬件实现通用转发功能**（基于流表的高速匹配+行动）。
- **流表(Flow Table)**被控制器计算和安装。
...
**SDN架构：SDN控制器(SDN Architecture: SDN Controller)**：

- **SDN控制器(SDN Controller)**（网络操作系统, Network OS）：
  - **维护网络状态信息**（全局网络视图）。
  - 通过上面的**北向API(Northbound API)**和网络控制应用交互。
  - 通过下面的**南向API(Southbound API)**和网络交换机交互。
  - **逻辑上集中(Logically Centralized)**，但是在实现上通常由于**性能、可扩展性、容错性以及鲁棒性**采用**分布式方法**。

**注意点**
- ⚠️ **逻辑集中 vs 物理分布**：控制器在概念上是单一实体，但实际部署常为分布式集群（避免单点故障、提升性能）。
- 📋 **术语提醒**：北向API(Northbound API)面向应用层，南向API(Southbound API)面向设备层。
...
**SDN架构：控制应用(SDN Architecture: Control Applications)**：

- **网络控制应用(Network Control Applications)**：
  - **控制的大脑(Brain of Control)**：采用下层提供的服务（SDN控制器提供的API），实现网络功能：
...
- 💡 **控制应用示例**：负载均衡应用可以根据实时流量动态调整流表，无需修改交换机固件。
...
**通用转发和SDN(Generalized Forwarding and SDN)**：

- 每个路由器包含一个**流表(Flow Table)**（被逻辑上集中的控制器计算和分发）。
- 控制平面计算流表并下发到数据平面，数据平面根据流表执行转发：
  - 逻辑集中式路由控制器(Logically-Centralized Routing Controller) → 控制平面(Control Plane) → 数据平面(Data Plane) → 本地流表(Local Flow Table) → 到达分组头部字段值(Values in Arriving Packet Header) → 输出链路(Output Link)。

**OpenFlow数据平面抽象(OpenFlow Data Plane Abstraction)**：

- **流(Flow)**：由分组（帧）头部字段所定义。
- **通用转发(Generalized Forwarding)**：简单的分组处理规则，包含四个核心要素：
  1. **模式(Pattern)**：将分组头部字段和流表进行匹配。
  2. **行动(Action)**：对于匹配上的分组，可以是**丢弃(Drop)**、**转发(Forward)**、**修改(Modify)**、将匹配的分组**发送给控制器(Send to Controller)**。
  3. **优先权(Priority)**：几个模式匹配了，优先采用哪个，消除歧义。
  4. **计数器(Counters)**：#bytes以及#packets（统计匹配流的数据包和字节数）。

- 路由器中的流表定义了路由器的**匹配+行动(Match+Action)**规则（流表由控制器计算并下发）。
...
- 📋 **术语提醒**：通用转发(Generalized Forwarding)、流(Flow)、匹配+行动(Match+Action)。
...
**OpenFlow：流表的表项结构(OpenFlow: Flow Table Entry Structure)**：

每个流表项(Flow Table Entry)包含三个核心部分：

1. **规则(Rule)**：匹配条件
2. **行动(Action)**：匹配后的处理动作
3. **统计(Stats)**：计数器（Packet + byte counters）

**行动(Action)类型**：

1. 向特定（可能多个）端口转发(Forward to specific port(s))
2. 分装分组转发给控制器(Encapsulate and forward to controller)
3. 丢弃分组(Drop packet)
4. 发送分组给处理管道/上报控制器(Send to processing pipeline / report to controller)
5. 修改字段(Modify fields)

**匹配字段(Match Fields)**（覆盖链路层到传输层）：

| 层次 | 匹配字段 |
|------|---------|
| **链路层(Link Layer)** | Switch Port, VLAN ID, MAC src, MAC dst, Eth type |
| **网络层(Network Layer)** | IP Src, IP Dst, IP Prot |
| **传输层(Transport Layer)** | TCP sport, TCP dport |
...
**例子1：基于目标的转发(Destination-Based Forwarding)**：

- **IP单播转发**：
  - Match: Switch Port=*, MAC src=*, MAC dst=*, Eth type=*, VLAN ID=*, IP Src=*, **IP Dst=51.6.0.8***, IP Prot=*, TCP sport=*, TCP dport=*
  - Action: **port6**
  - 含义：IP数据报目标地址是51.6.0.8应该被通过端口6转发。

- **防火墙示例（基于端口过滤）**：
  - Match: Switch Port=*, MAC src=*, MAC dst=*, Eth type=*, VLAN ID=*, IP Src=*, IP Dst=*, IP Prot=*, TCP sport=*, **TCP dport=22**
  - Action: **drop**
  - 含义：不转发（阻塞）所有具有目标TCP端口号是22的分组。

- **防火墙示例（基于源IP过滤）**：
  - Match: Switch Port=*, MAC src=*, MAC dst=*, Eth type=*, VLAN ID=*, **IP Src=128.119.1.1***, IP Dst=*, IP Prot=*, TCP sport=*, TCP dport=*
  - Action: **drop**
  - 含义：所有由128.119.1.1发送的分组都应该被阻塞。

**例子2：基于层2目标的转发(Layer-2 Destination-Based Forwarding)**：

- Match: Switch Port=*, **MAC src=22:A7:23:11:E1:02***, MAC dst=*, Eth type=*, VLAN ID=*, IP Src=*, IP Dst=*, IP Prot=*, TCP sport=*, TCP dport=*
- Action: **port3**
