...
- **可靠的、按顺序的字节流(Reliable, In-Order Byte Stream)**：**没有报文边界**
- **管道化(Pipelining)**：TCP拥塞控制(Congestion Control)和流量控制(Flow Control)设置窗口大小
- **发送和接收缓存(Send & Receive Buffers)**：进程写数据到TCP发送缓存，TCP从缓存取数据组成报文段；接收方TCP将数据放入接收缓存，进程从缓存读取
- **全双工数据(Full-Duplex Data)**：在同一连接中数据流双向流动；**MSS(Maximum Segment Size)**：最大报文段大小
...
| 序号(Sequence Number) | 对**字节**计数(而不是报文段) |
| 确认号(Acknowledgement Number) | 期望收到的下一个字节的序号 |
| 首部长度(Header Length) | 以32比特字为单位 |
| 保留(Reserved) / 未用 | |
| **URG** | 紧急数据(通常不用) |
| **ACK** | ACK #合法 |
| **PSH** | 马上推出数据(通常不用) |
...
- 📋 术语：MSS(Maximum Segment Size) 是最大报文段大小，受限于链路层帧大小
...
- **确认号(Acknowledgement Number)**：期望从另一方收到的下一个字节的序号，采用**累积确认(Cumulative ACK)**
- **Q: 接收方如何处理乱序的报文段-没有规定(RFC没有明确规定)**

**教材示例：简单telnet场景**

Host A 发送 `Seq=42, ACK=79, data='C'` 给 Host B：
- Host B 收到后回显 'C'，发送 `Seq=79, ACK=43, data='C'`（确认收到42，期望下一个43）
- Host A 收到回显后发送 `Seq=43, ACK=80`（确认收到79及之前）
...
TCP在IP不可靠服务的基础上建立了**rdt(Reliable Data Transfer)**：
...
- **状态：等待事件(Wait for Event)**
  ...
  - **事件1：从应用层接收数据(data received from application above)**
    ...
    - 将报文段传给IP（即"发送"）
    - 更新：`NextSeqNum = NextSeqNum + length(data)`
    - 下一状态：保持等待事件
