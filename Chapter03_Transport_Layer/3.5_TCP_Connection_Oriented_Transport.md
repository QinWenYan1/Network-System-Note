...
- **事件1：从应用层接收数**
    - 动作`seq # = NextSeqNum`：创建报文段，为报文段首字节的字节流编号 
...
2. **超时过早**
   ...
   - 随后 ACK=120 到达，SendBase 更新为120（即使92被重传了，也不会重传100，因为**ACK表达的意思是对顺序到来的下一个字节的期待**）

3. **超时过晚**
   - Host A 发送 Seq=92(8字节) 和 Seq=100(20字节)
   - ACK = 100丢失，ACK = 120 正确到达
   - 但这时发送方的Seq = 92还没有超时
   - 这个时候由于TCP的**累计确认**性质，sendbase直接到达120，ACK表明了自己想得道最后确认的字节119后一位120字节开始的分组
...
| 接收方的事件 | TCP接收方动作 |
|-------------|--------------|
| 所期望序号的报文段**按序到达**。所有在期望序号之前的数据都已经被确认 | **延迟的ACK**。对另一个按序报文段的到达最多等待500ms。如果下一个报文段在这个时间间隔内没有到达，则发送一个ACK。**能搭便车就搭便车** |
| 有期望序号的报文段**到达**。另一个按序报文段等待发送ACK | 立即发送单个累积ACK，以确认两个按序报文段。**搭上便车了** |
| 比期望序号大的报文段**乱序到达**。检测出数据流中的间隔 | 立即发送**ACK**，指明期待字节的序号，**表示哪里你需要补充给我** |
| **能部分或完全填充接收数据间隔**(gap)的报文段到达。 | 若该报文段起始于间隔(gap)的低端，则立即发送期待字节的ACK。**表示哪里你需要补充给我** |
...
- 接收方通过**捎带**的方式在其向发送方的TCP段头部的**rwnd**字段"通告"其空闲buffer大小

**捎带定义**
* **捎带(Piggybacking)**：发送数据时顺便把确认信息"搭便车"一起发过去，不用单独发一个ACK报文，省带宽。
...
在正式交换数据之前，发送方和接收方<b>握手(Handshake)</b>建立通信关系：
- 同意建立连接（每一方都知道对方愿意建立连接）
- 同意连接参数（初始序号、窗口大小等）

Question: 在网络中，**为什么2次握手不行？**
...
1. **半连接(Half-Connection)**：只在服务器端维护了连接
   ...
   - 由于超时，客户端重传 `req_conn(x)`
   - 客户端此时终止连接
   - 之后服务器收到重传的请求，仍认为连接存在，在单方面断开时还在维护
...
- 接收`SYNACK(x)`：表明服务器是活跃的；发送SYNACK的ACK；该报文可能包含C→S的数据
- 接收`ACK(y)`：表明客户端是活跃的
...
- ⚠️关键区别：**两次握手是服务器一厢情愿先把门开了，旧数据趁机溜进来；三次握手必须双方都点头才开门(确认的确认)，对方不点头门就没开，旧数据到了也无处可去。**
