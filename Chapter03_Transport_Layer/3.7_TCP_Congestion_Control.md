...
**理论**
- **端到端的拥塞控制机制 (End-to-End Congestion Control)**：
  - **路由器不向主机发送有关拥塞的反馈信息** —— 路由器负担较轻，符合网络核心简单的 **TCP/IP架构原则**
  - **端系统 (End Systems)** 根据自身得到的信息（如丢包、延迟），判断是否发生拥塞，从而采取动作
- **拥塞控制的几个问题 (Key Questions)**：
  1. **如何检测拥塞 (How to Detect Congestion)**：
     - **轻微拥塞 (Mild Congestion)**：网络还能传一部分数据
     - **拥塞 (Severe Congestion)**：网络严重堵塞
...
**一、拥塞窗口 CongWin (Congestion Window)**
- 维持一个拥塞窗口的值：**CongWin**
...
**二、CongWin的动态调整机制 (3-125)**
- **CongWin是动态的**，是感知到的网络拥塞程度的函数
- **超时或者3个重复ACK → CongWin↓**：
  - **超时时**：CongWin降为 **1 MSS**，进入 **SS阶段** 然后再倍增到 CongWin/2（每个RTT），从而进入 **CA阶段**
  - **3个重复ACK**：CongWin降为 **CongWin/2**，进入 **CA阶段**
- **否则（正常收到ACK）→ CongWin↑**：
  - **SS阶段**：加倍增加（每个RTT）
  - **CA阶段**：线性增加（每个RTT）

**三、TCP拥塞控制和流量控制的联合动作 (3-126)**
...
- 📋 **术语提醒**：MSS = Maximum Segment Size（最大段大小），SS = Slow Start（慢启动），CA = Congestion Avoidance（拥塞避免）
